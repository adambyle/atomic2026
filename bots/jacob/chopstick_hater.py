...
class SimState:
    ...
    def apply_plays(self, plays: list):
        ...
        -1 means this player has no card to play (empty hand guard).
        Update tableaux, puddings, then rotate hands.
        """
        for i, idx in enumerate(plays):
            if idx < 0 or idx >= len(self.hands[i]):
                continue  # empty hand or sentinel — skip
...
def mcts_search(root_state: SimState, time_budget_s: float = 0.95,
    ...
    while time.monotonic() < deadline:
        ...
        # ── Expansion ──────────────────────────────────────────────────────
        if node.untried and not state.is_round_over():
...
def _build_plays(state: SimState, my_move: int) -> list:
    ...
    for i in range(state.player_count):
        if i == state.my_idx:
            # Guard: if somehow our hand is smaller than the move index, clamp
            safe_move = my_move if my_move < len(state.hands[i]) else 0
            plays.append(safe_move)
        else:
            hand = state.hands[i]
            if not hand:
                # Hand is empty — skip this player (round effectively over for them)
                plays.append(-1)  # sentinel; apply_plays must handle this
                continue
...
class ChopstickHater:
    ...
    #         i, j = best_pair
    #         # Safety: don't try to play Chopsticks card via CHOPSTICKS command.
    #         # Chopsticks card in hand should be played normally (PLAY), not spent.
    #         if "Chopsticks" in (hand[i], hand[j]):
    #             return None
    #         return best_pair
    #     return None
        ...
        try:
            ...
            while True:
                ...
                if msg.startswith("JOINED"):
                    ...
                    # ── Safety guard: ensure index is in range before sending ──
                    # Rare state-drift bugs can produce an out-of-range index.
                    # Fall back to index 0 rather than send an illegal move.
                    ...
                    if action.startswith("CHOPSTICKS"):
                        parts = action.split()
                        i, j = int(parts[1]), int(parts[2])
                        # Spend the chopsticks card: remove one from our tableau.
                        # Per the rules, it returns to hand rotation (server handles
                        # the rotation — we just stop tracking it as "ours").
                        t = self.gk.players[name].tableau
                        if "Chopsticks" in t:
                            t.remove("Chopsticks")
                        # Both played cards land in our tableau
                    ...
                    plays = parse_played(msg)  # {name: [card, ...]}
                    # Update opponent tableaux
                    for pname, cards in plays.items():
                        if pname == name:
                            continue  # our tableau already updated above
                        if pname not in self.gk.players:
                            continue
                        opp = self.gk.players[pname]
                        used_chops = len(cards) >= 2
                        if used_chops:
                            # Opponent spent their chopsticks — remove from their tableau
                            if "Chopsticks" in opp.tableau:
                                opp.tableau.remove("Chopsticks")
                        for card in cards:
                            opp.tableau.append(card)
                            if card == "Pudding":
                                opp.puddings += 1
                    # Infer next player's hand from our previous hand.
                    # Use the first card we played as what to remove.
                    next_seat = self.gk.next_seat()
                    if next_seat and self.gk.prev_hand:
                        my_cards = plays.get(name, [])
                        my_first_play = my_cards[0] if my_cards else ""
                        inferred = list(self.gk.prev_hand)
                        if my_first_play and my_first_play in inferred:
                            inferred.remove(my_first_play)
                    ...
                    # If server rejects our move, log it. E007 means we tried
                    # chopsticks without having them — clear any stale tableau entry.
                    if "E007" in msg:
                    ...
                    for nm, sc in sorted(scores.items(), key=lambda x: -x[1]):
                        tag = " ← Chopstick Hater" if nm == name else ""
                        print(f"  {nm}: {sc}{tag}")
                    break
